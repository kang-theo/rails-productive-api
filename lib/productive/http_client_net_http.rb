...
module Productive
  module HttpClient
    ...
    @@auth_info = ProductiveConf.auth_info
    ...
    # HttpClient.get('...')
    def self.get(req_params, opt = {})
      url = "#{@@endpoint}/#{req_params}"
    ...
    class Request
      ...
      def delete(url, _payload = nil, opt = {})
      ...
      def get_response(url, method = 'GET', payload = nil, opt = {})
        ...
        until false
          ...
          # end
          headers.merge!(opt.to_h) unless opt.nil?

          if use_cache?
            cache_entry = cache[uri.to_s]
            if cache_entry
              logger.info "found cache entry for >#{uri}<"
              if cache_entry['etag']
